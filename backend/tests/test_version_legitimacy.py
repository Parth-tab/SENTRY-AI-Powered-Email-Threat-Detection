...
def test_version_legitimacy_backed_by_highest_git_tag():
    """
    MV-1: Asserts that SENTRY's unified version (1.2.1) is mathematically
    backed by the highest release tag in git history (v1.2.1).
    ...
    assert version_data["version"] == "1.2.1"
    assert version_data["highest_tag"] == "v1.2.1"
    assert version_data["highest_tag_version"] == "1.2.1"
    assert version_data["legitimate"] is True, "Version 1.2.1 must be backed by git tag v1.2.1"

    valid, msg = validate_facts.verify_version_legitimacy(version_data)
    assert valid is True
    assert "backed by git tag v1.2.1" in msg
...
def test_mutation_kill_unbacked_version_bump_fails_validator():
    ...
    the unbacked version ('9.9.9') and the highest existing tag ('v1.2.1').
    """
    ...
    assert "v1.2.1" in err_msg, f"Expected highest tag 'v1.2.1' in error: {err_msg}"
...
def test_mutation_kill_stale_tags_fails_validator(monkeypatch):
    """
    MV-1 Mutation Kill:
    If the git repository only contains older tags (e.g. v1.1.1) while the codebase
    declares 1.2.1, the legitimacy check must fail.
    """
    monkeypatch.setattr(validate_facts, "get_highest_git_release_tag", lambda: "v1.1.1")
    monkeypatch.delenv("SENTRY_PRE_TAG_STAGING", raising=False)

    version_data = validate_facts.compute_app_version()
    assert version_data["version"] == "1.2.1"
    assert version_data["highest_tag"] == "v1.1.1"
    assert version_data["legitimate"] is False

    valid, err_msg = validate_facts.verify_version_legitimacy(version_data)
    assert valid is False
    assert "1.2.1" in err_msg
    assert "v1.1.1" in err_msg

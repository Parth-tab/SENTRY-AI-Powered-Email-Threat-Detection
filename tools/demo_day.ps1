...
$repo = "E:\SENTRY"
Set-Location $repo
Write-Host "=== SENTRY DEMO APPLIANCE — $(Get-Date -Format 'yyyy-MM-dd HH:mm') ==="
...
# [4/6] Data layer
Write-Host "`n[4/6] Postgres/Redis/Neo4j via Docker (waiting for health)..."
docker compose -f "$repo\docker-compose.yml" up -d --wait
if ($LASTEXITCODE -ne 0) {
    Write-Warning "Windows docker path failed; trying WSL path..."
    wsl docker compose -f /mnt/e/SENTRY/docker-compose.yml up -d
    Start-Sleep -Seconds 10
}
if ($PreflightOnly) { Write-Host "`nPreflight-only — stopping before app boot."; exit 0 }
...
$args = @("--user-data-dir=$repo\.demo-profile","--start-fullscreen",
          "--window-size=1920,1080","--no-first-run","--no-default-browser-check",
          "http://localhost:3000")
if (Test-Path $chrome) { Start-Process $chrome $args }
elseif (Test-Path $edge) { Start-Process $edge $args }
else { Start-Process "http://localhost:3000" }

Write-Host @"

BOOT COMPLETE — 15/15 verified.
  Dashboard : http://localhost:3000
  Demo flow : docs/DEMO_SCRIPT.md (timings printed on it — narrate from it)
  API docs  : http://localhost:8000/docs (mention; open only if judges ask)
  MID-DEMO FAILURE: tools\cleanup.ps1 -> re-run this script (~2 min),
                    or cut to backup video without apology.
"@

...
with sync_playwright() as p:
    ...
    # 1. Open Dashboard on Port 3001
    print("[1] Opening http://localhost:3001...")
    page.goto("http://localhost:3001", wait_until="networkidle")
    time.sleep(2)

    # Dashboard screenshot with all 3 emails in table
    page.screenshot(path=str(SCREENSHOT_DIR / "1_soc_dashboard_live.png"), full_page=True)
    print("[+] 1_soc_dashboard_live.png captured")

    # 2. Click on the first critical threat (SBI KYC Phishing) row
    row = page.locator('tbody tr').first
    if row.count() > 0:
        row.click()
        time.sleep(2)
        page.screenshot(path=str(SCREENSHOT_DIR / "2_email_forensic_analyzer_modal.png"))
        print("[+] 2_email_forensic_analyzer_modal.png captured")
    ...
    if map_btn.count() > 0:
        ...
        print("[+] 3_relay_world_map_live.png captured")
    ...
    if graph_btn.count() > 0:
        ...
        print("[+] 4_campaign_network_graph_live.png captured")
    ...
    if vault_btn.count() > 0:
        ...
        print("[+] 5_forensic_vault_verified_live.png captured")

    browser.close()
    print("[*] All gallery screenshots updated!")

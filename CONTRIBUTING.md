...
1. **Nothing merges red.** CI runs: ruff + pytest (branch coverage) + the
   15-check E2E golden harness (boots the real stack, drives headless
   Chromium) + the 12-dimension GAUNTLET battery.
...
4. **Security-relevant changes** (sanitization, auth, headers, upload path)
   require a test in the same PR demonstrating the vulnerability is closed.

## Verification loop for any change
```bash
pytest backend/tests -v                      # unit/integration suite
python tools/verify_sentry.py --start        # end-to-end golden harness (15/15)
...
(harness output or screenshot), and confirmation of the loop above.
